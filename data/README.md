...
- `pipeline_source_status_latest.csv`: estado das fontes no último ciclo registado, usado para assinalar no boletim quando alguma fonte não pôde ser atualizada.
...
- O workflow `Pipeline diário Matosinhos` tenta executar nas janelas 09:20-11:59 e 15:20-17:59 em Europe/Lisbon, com disparos repetidos de 15 em 15 minutos para compensar atrasos ou falhas pontuais do agendamento do GitHub Actions. Cada ciclo diário é executado uma única vez através de `workflow_run_markers.csv`. O pipeline atualiza as fontes em etapas separadas (IPMA meteorologia, SNS/INSA, Clima Extremo, avisos IPMA, erro das previsões IPMA e QualAr) e gera o relatório através de `run_daily_pipeline.R`, que regista erros por fonte em vez de abortar o ciclo. O envio de email está desativado por agora; o código fica mantido para reativação futura, depois do commit do relatório.

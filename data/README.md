...
- O workflow `Recolher IPMA Matosinhos` corre em modo completo às 10:15 UTC e 20:15 UTC, pouco depois das duas disponibilizações diárias indicadas pelo IPMA. Às 14:15 UTC corre em modo leve, atualizando apenas observações de estações e avisos IPMA. O modo completo atualiza as secções `Temperatura DSP`, `Onda de Calor`, `Stress térmico UTCI`, `Índice UV` e `Avisos IPMA` no ficheiro diário do dia de execução quando há previsão atual, ou na primeira data disponível quando não há.
